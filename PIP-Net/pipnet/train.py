...
def calculate_loss(proto_features, pooled, out, ys1, align_pf_weight, t_weight, unif_weight, cl_weight,
    ...
    if not pretrain:
        logits = torch.log1p(out ** net_normalization_multiplier)   # (2B, K)
        class_loss = criterion(logits, ys.float())                  # BCE 内部 sigmoid

        loss = cl_weight * class_loss if finetune else loss + cl_weight * class_loss

    acc = 0.
    if not pretrain:
        y_hat = (torch.sigmoid(out) > 0.5).float()  # 未归一化 logits 直接过 sigmoid
        acc = (y_hat == ys).float().mean().item()  # element‑wise 命中率
    if print:
        with torch.no_grad():
            if pretrain:
                train_iter.set_postfix_str(
