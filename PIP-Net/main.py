...
from pipnet.train_multi import train_pipnet
from pipnet.test_multi import eval_pipnet
...
def run_pipnet(args=None):
    ...
    # Obtain the dataset and dataloaders *****改前两个
    ...
    with torch.no_grad():
        if args.state_dict_dir_net != '': #是否加载预训练模型
            epoch = 0
            checkpoint = torch.load(args.state_dict_dir_net,map_location=device)
            net.load_state_dict(checkpoint['model_state_dict'],strict=True) 
            print("Pretrained network loaded", flush=True)
            net.module._multiplier.requires_grad = False #冻结
            try:
    ...
    with torch.no_grad():
        ...
        args.wshape = wshape #needed for calculat # Initialize or load model
    with torch.no_grad():
        if args.state_dict_dir_net != '': #是否加载预训练模型
            epoch = 0
            checkpoint = torch.load(args.state_dict_dir_net,map_location=device)
            ...
            print("Classification layer initialized with mean", torch.mean(net.module._classification.weight).item(), flush=True) # Initialize or load model
    with torch.no_grad():
        if args.state_dict_dir_net != '': #是否加载预训练模型
            epoch = 0
            checkpoint = torch.load(args.state_dict_dir_net,map_location=device)
            ...
            print("Classification layer initialized with mean", torch.mean(net.module._classification.weight).item(), flush=True)ing image patch size
    ...
    lrs_pretrain_net = []
    # PRETRAINING PROTOTYPES PHASE 预训练阶段
    ...
    for epoch in range(1, args.epochs + 1):                      
        ...
        with torch.no_grad():
            ...
            plt.clf()
            plt.plot(lrs_net)
            plt.savefig(os.path.join(args.log_dir,'lr_net.png')) #学习率可视化
            plt.clf()
    ...
    # 只对 CUB-200-2011 数据集启用，因为只有 CUB 数据集提供 parts/parts.txt 和 parts/part_locs.txt。
    if args.dataset == 'CUB-200-2011':
        ...
        cubthreshold = 0.5  #用于评估原型 patch 的“相似度阈值”，大于此阈值的 patch 才认为是这个原型的正激活。
        ...
        #另一种评估方式：基于阈值激活：与 top-K 不同，它找出 所有激活值 > 0.5 的 patch，统计是否集中在某个语义部位。
        cubthreshold = 0.5
