...
num = len(images)
for k in range(num):
    parts = images[k][0].split(' ')
    id = parts[0]
    fn = ' '.join(parts[1:])  # 合并剩余部分作为文件名
    id = int(id)
    ...
    if int(split[k][0][-1]) == 1:
        ...
        img = Image.open(os.path.join(os.path.join(path, 'images'), ' '.join(parts[1:]))).convert('RGB')
        x, y, w, h = bboxes[id]
        cropped_img = img.crop((x, y, x + w, y + h))
        cropped_img.save(
            os.path.join(os.path.join(train_save_path, file_name), ' '.join(parts[1:]).split('/')[1]))
        print('%s' % ' '.join(parts[1:]).split('/')[1])
    else:
        ...
        cropped_img = img.crop((x, y, x + w, y + h))
        cropped_img.save(
            os.path.join(os.path.join(test_save_path, file_name), ' '.join(parts[1:]).split('/')[1]))
        print('%s' % ' '.join(parts[1:]).split('/')[1])
...
for k in range(num):
    ...
    if int(split[k][0][-1]) == 1:
        ...
        img.save(os.path.join(os.path.join(train_save_path, file_name), ' '.join(parts[1:]).split('/')[1]))

        print('%s' % ' '.join(parts[1:]).split('/')[1])
    else:
        if not os.path.isdir(test_save_path + file_name):
            os.makedirs(os.path.join(test_save_path, file_name))
        shutil.copy(path + 'images/' + ' '.join(parts[1:]),
                    os.path.join(os.path.join(test_save_path, file_name), ' '.join(parts[1:]).split('/')[1]))
        print('%s' % ' '.join(parts[1:]).split('/')[1])
time_end = time.time()
print('ChestMNIST, %s!' % (time_end - time_start))
