                    ...
                    for idx, score in topks[p]:
                        if idx == i:
                            ...
                            with torch.no_grad():
                                ...
                                if outmax.item() <= 0.:  # 支持多标签时可能出现负 logit
                                    abstained += 1
                            ...
                            if (c_weight > 1e-10) or ('pretrain' in foldername):
                                ...
                                img_to_open = imgs[i]
                                if isinstance(img_to_open, (tuple, list)):  # dataset returns (path, label)
                                    imglabel = img_to_open[1]
                                    img_to_open = img_to_open[0]

                                    # ── 将多热标签转成人类可读字符串，用于文件名 ──
                                    if isinstance(imglabel, torch.Tensor):  # multi‑hot: shape (K,)
                                        act_idx = imglabel.nonzero(as_tuple=True)[0].tolist()  # e.g. [0,2,5]
                                        label_str = '_'.join(map(str, act_idx)) or 'none'
                                    else:  # 单整数标签
                                        label_str = str(imglabel)
                                else:
                                    label_str = 'unknown'
            ...
            if c_weight>0:
                ...
                if found_max > 0.5:
                    ...
                    image.save(os.path.join(
                        save_path,
                        f'p{p}_{label_str}_{round(found_max, 2)}_{os.path.basename(img_to_open).split(".")[0]}_rect.png'
                    ))
